...
document.addEventListener('keydown',e=>{if(e.key==='Escape'){closeTournamentPicker();closeMatchDetail();closeTeamDetail();}});
...
function teamPlayers(teamId){
  return Object.values(state.players||{}).filter(p=>String(p.teamId||'')===String(teamId)&&String(p.status||'active')!=='inactive').sort((a,b)=>Number(a.number||999)-Number(b.number||999)||String(a.name||'').localeCompare(String(b.name||''),'es'));
}
function playerStats(teamId){
  const out={};
  for(const evs of Object.values(state.events||{})) for(const e of Object.values(evs||{})){
    const tid=String(e.team||e.equipo||''); if(tid!==String(teamId)) continue;
    const raw=String(e.player||e.playerName||e.jugador||'').trim(); if(!raw) continue;
    const key=normalize(raw); if(!out[key]) out[key]={goals:0,yellow:0,red:0};
    const type=String(e.type||'').toLowerCase();
    if(type==='gol') out[key].goals++; if(type==='amarilla') out[key].yellow++; if(type==='roja') out[key].red++;
  }
  return out;
}
function renderTeamModal(teamId){
  const modal=$('#teamDetailModal'); if(!modal)return;
  const t=team(teamId); if(!t)return;
  state.selectedTeamId=teamId;
  const g=String(t.group||'').toUpperCase();
  const stage=firstGroupStage();
  const row=stage?standingsFor(g,stage.id).find(x=>String(x.id)===String(teamId)):null;
  const roster=teamPlayers(teamId), stats=playerStats(teamId);
  const playersHtml=roster.length?roster.map(p=>{
    const ps=stats[normalize(p.name||'')]||{goals:0,yellow:0,red:0};
    const photo=p.photo?`<img src="${esc(p.photo)}" alt="">`:'<span class="team-player-avatar-fallback">⚽</span>';
    return `<article class="team-player-card"><div class="team-player-photo">${photo}</div><div class="team-player-number">${Number(p.number||0)||'—'}</div><div class="team-player-main"><b>${esc(p.name||'Jugador')}</b><span>${esc(p.position||'Jugador')}${p.captain?' · © Capitán':''}</span><small>⚽ ${ps.goals} &nbsp; 🟨 ${ps.yellow} &nbsp; 🟥 ${ps.red}</small></div></article>`;
  }).join(''):'<div class="team-roster-empty"><span>👥</span><b>No hay jugadores registrados</b><small>El administrador todavía no ha registrado la plantilla de este equipo.</small></div>';
  modal.innerHTML=`<div class="modal-backdrop" data-close-team></div><div class="team-detail-box" role="dialog" aria-modal="true" aria-labelledby="teamDetailTitle"><button class="modal-close public-modal-close" type="button" aria-label="Cerrar" data-close-team>×</button><header class="team-detail-head"><div class="team-detail-brand">${teamLogo(teamId,'team-detail-logo')}</div><div><span class="kicker">GRUPO ${esc(g||'—')}</span><h2 id="teamDetailTitle">${esc(t.name||teamId)}</h2><p>${esc(t.description||'Plantilla y estadísticas del equipo')}</p></div></header><div class="team-stat-grid"><div><b>${row?.pj||0}</b><span>PJ</span></div><div><b>${row?.pg||0}</b><span>PG</span></div><div><b>${row?.pe||0}</b><span>PE</span></div><div><b>${row?.pp||0}</b><span>PP</span></div><div><b>${row?.gf||0}</b><span>GF</span></div><div><b>${row?.gc||0}</b><span>GC</span></div><div><b>${row?.dg>0?'+':''}${row?.dg||0}</b><span>DG</span></div><div><b>${row?.pts||0}</b><span>PTS</span></div></div><section class="team-roster-section"><div class="detail-section-head"><div><span class="kicker">PLANTILLA</span><h3>Jugadores registrados</h3></div><span>${roster.length} jugador${roster.length===1?'':'es'}</span></div><div class="team-roster-grid">${playersHtml}</div></section></div>`;
  modal.classList.add('open'); modal.setAttribute('aria-hidden','false'); document.body.classList.add('modal-open');
}
function closeTeamDetail(){const modal=$('#teamDetailModal');if(!modal)return;modal.classList.remove('open');modal.setAttribute('aria-hidden','true');state.selectedTeamId=null; if(!document.querySelector('.public-match-modal.open'))document.body.classList.remove('modal-open');}
function bindTeamInteractions(){
  const standings=$('#standings'); if(!standings)return;
  standings.querySelectorAll('[data-team-id]').forEach(el=>{el.addEventListener('click',()=>renderTeamModal(el.dataset.teamId));el.addEventListener('keydown',e=>{if(e.key==='Enter'||e.key===' '){e.preventDefault();renderTeamModal(el.dataset.teamId);}});});
}
function renderStandings(){const stage=firstGroupStage();if(!stage){$('#standings').innerHTML='<div class="empty">Este torneo no tiene una fase de grupos publicada.</div>';return;}const groups=[...new Set(Object.values(state.teams).map(t=>t.group).filter(Boolean))].sort((a,b)=>String(a).localeCompare(String(b),'es'));$('#standings').innerHTML=groups.length?groups.map(g=>`<div class="standings-card"><div class="card-title"><div><span class="group-badge ${String(g).toUpperCase()==='B'?'group-b':''}">${esc(g)}</span><div><b>GRUPO ${esc(g)}</b><small>Todos contra todos</small></div></div><span>${esc(stage.name||'Fase de grupos')}</span></div><div class="table-wrap"><table><colgroup><col class="col-pos"><col class="col-team"><col><col><col><col><col><col></colgroup><thead><tr><th>#</th><th>Equipo</th><th>PJ</th><th>PG</th><th>PE</th><th>PP</th><th>DG</th><th>PTS</th></tr></thead><tbody>${standingsFor(g,stage.id).map((x,i)=>`<tr><td><span class="pos ${i<2?'qualified':''}">${i+1}</span></td><td><div class="table-team"><button type="button" class="table-team team-click" data-team-id="${esc(x.id)}" aria-label="Ver ficha de ${esc(x.name)}">${teamLogo(x.id,'team-logo tiny')}<b>${esc(x.name)}</b><span>›</span></button></div></td><td>${x.pj}</td><td>${x.pg}</td><td>${x.pe}</td><td>${x.pp}</td><td class="${x.dg>=0?'positive':'negative'}">${x.dg>0?'+':''}${x.dg}</td><td><strong>${x.pts}</strong></td></tr>`).join('')}</tbody></table></div></div>`).join(''):'<div class="empty">Asigna grupos a los equipos para mostrar posiciones.</div>';bindTeamInteractions();}

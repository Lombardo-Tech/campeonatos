...
function renderAdminByeInfo(){
  const box=$('#adminByeInfo');if(!box)return;
  if(S.dateFilter!=='__byes__'){box.hidden=true;box.innerHTML='';return;}
  const matches=Object.values(S.matches||{});const byDate=new Map();
  matches.forEach(m=>{const k=dateKeyForMatch(m);if(!k||k==='::')return;if(!byDate.has(k))byDate.set(k,{label:dateLabelForMatch(m),groups:new Set(),round:Number(m.roundNumber||0)});if(m.group)byDate.get(k).groups.add(String(m.group).toUpperCase());});
  const out=[];
  byDate.forEach((x,k)=>{x.groups.forEach(g=>{const ids=Object.entries(S.teams||{}).filter(([,t])=>String(t.group||'').toUpperCase()===g).map(([id])=>id);const played=new Set();matches.filter(m=>dateKeyForMatch(m)===k&&String(m.group||'').toUpperCase()===g).forEach(m=>{if(m.local)played.add(m.local);if(m.visitor)played.add(m.visitor);});const free=ids.filter(id=>!played.has(id));if(free.length)out.push({key:k,label:x.label,round:x.round,group:g,names:free.map(id=>S.teams[id]?.name||id)});});});
  out.sort((a,b)=>b.round-a.round||String(a.label).localeCompare(String(b.label),'es')||a.group.localeCompare(b.group));
  box.hidden=false;
  box.innerHTML=out.length?`<div class="bye-info-head"><span>💤</span><div><b>Equipos libres</b><small>Descansos registrados por fecha</small></div></div><div class="bye-info-list">${out.map(x=>`<div class="bye-date"><strong>${esc(x.label)}</strong><div class="bye-names">${x.names.map(n=>`<div>${esc(n)} · Grupo ${esc(x.group)}</div>`).join('')}</div></div>`).join('')}</div>`:'<div class="bye-info-head"><span>💤</span><div><b>Equipos libres</b><small>No hay equipos libres registrados.</small></div></div>';
}
...
function renderMatches(){
  ...
  renderAdminByeInfo();
  const byeMode=S.dateFilter==='__byes__';
  const tableCard=box.closest('.table-card');
  if(byeMode){ if(tableCard)tableCard.hidden=true; box.innerHTML=''; return; }
...
async function finishLiveMatch(){const id=$('#resultMatch').value;if(!id)return;const m=S.matches[id]||{};if(String(m.status||'').toLowerCase()!=='en juego')return msg('El partido no está en juego.');const added=Math.max(0,Math.min(30,Number($('#addedTime')?.value||0)||0));const minute=liveMinuteLabelForAdmin({...m,liveAddedTime:added});if(!confirm(`¿Finalizar el partido en ${minute}${added?` con +${added}`:''}?`))return;const finalPatch={status:'finalizado',liveStartedAt:null,periodStartedAt:null,liveAddedTime:null,secondHalfMinute:minute,secondHalfAddedTime:added,finishedAt:now(),updatedAt:now()};await update(ref(db,`partidos/${S.tid}/${id}`),finalPatch);try{await downloadMatchActPDF({tournament:currentTournament(),match:{...m,...finalPatch},teams:S.teams,events:S.events?.[id]||{}});}catch(err){console.error('PDF acta:',err);msg('Partido finalizado. No se pudo descargar el PDF automáticamente.');}const finalStage=stages().find(s=>s.type==='final'||String(s.id||'').toLowerCase()==='final');if(finalStage&&m.stageId===finalStage.id){const hs=Number(m.homeScore||0),as=Number(m.awayScore||0);await update(ref(db,`tournaments/${S.tid}`),{status:'finished',finishedAt:now(),championTeamId:hs>as?m.local:as>hs?m.visitor:null});}closeResult(true);msg('Partido finalizado.');}
...
function initModules(){document.querySelectorAll('[data-module-tab]').forEach(b=>b.addEventListener('click',()=>switchModuleTab(b.dataset.moduleTab)));$('#playerForm')?.addEventListener('submit',savePlayer);$('#clearPlayer')?.addEventListener('click',clearPlayer);$('#registrationForm')?.addEventListener('submit',saveRegistration);$('#venueForm')?.addEventListener('submit',saveVenue);$('#newsForm')?.addEventListener('submit',saveNews);$('#sponsorForm')?.addEventListener('submit',saveSponsor);$('#awardForm')?.addEventListener('submit',saveAward);$('#printMatchAct')?.addEventListener('click',async()=>{const id=$('#minutesMatch')?.value||'';const m=S.matches[id];if(!m)return msg('Selecciona un partido finalizado.');const btn=$('#printMatchAct');btn.disabled=true;btn.textContent='Generando PDF...';try{await downloadMatchActPDF({tournament:currentTournament(),match:m,teams:S.teams,events:S.events?.[id]||{}});msg('Acta abierta para imprimir/guardar como PDF.');}catch(err){console.error(err);msg(err?.message||'No se pudo generar el PDF.');}finally{btn.disabled=false;btn.textContent='📄 Descargar acta PDF';}});}
...
function editModule(k,id){const x=moduleStore[k]?.[id];if(!x)return;if(k==='jugadores'){['playerTeam','playerName','playerNumber','playerPosition','playerPhoto','playerStatus'].forEach((id,i)=>{});$('#playerId').value=id;$('#playerTeam').value=x.teamId||'';$('#playerName').value=x.name||'';$('#playerNumber').value=x.number||'';$('#playerPosition').value=x.position||'Arquero';$('#playerPhoto').value=x.photo||'';$('#playerCaptain').checked=!!x.captain;$('#playerStatus').value=x.status||'active';}
